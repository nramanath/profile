...
1. Verify the current git branch using `git branch --show-current`. If the current branch is `main` or `master`, use the `run_command` tool to create and checkout a new branch (e.g., `git checkout -b <new-branch-name>`). Do not make commits directly to the main branch.
